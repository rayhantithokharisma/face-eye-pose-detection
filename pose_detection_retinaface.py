...
if __name__ == "__main__":
    ...
    video_save = True# do you want to record video?
    fps = 10.
    ...
    while (True): 
        ...
        if len(bboxes) > 0:
            ...
            if are_coordinates_in_frame(image_rgb, bb, lmarks_5):
                ...
                cv2.putText(image_rgb, "Face Pose", (10, 40), font, 0.8, red, 2) 
                cv2.putText(image_rgb, "Method 1", (10, 60), font, 0.4, green, 2) 
                ...
                cv2.putText(image_rgb, "Yaw: {0:.2f} (-100 to +100)".format(
                    find_yaw(lmarks_5)), (10, 100), font, 0.4, red, 1)
                cv2.putText(image_rgb, "Pitch: {0:.2f} (0 to 4)".format(
                    find_pitch(lmarks_5)), (10, 120), font, 0.4, green, 1)
                
                angle, Xfrontal, Yfrontal = find_pose(lmarks_5)
                cv2.putText(image_rgb, "Method 2", (10, 180), font, 0.4, red, 2)
                cv2.putText(image_rgb, "Roll: {0:.2f} degrees".format(angle), 
                            (10,200), font, 0.4, green, 1)
        ...
        cv2.putText(image_rgb, "Press Q to quit.", (10, 460), font, 0.4, red, 1)
        image_bgr = cv2.cvtColor(image_rgb, cv2.COLOR_RGB2BGR)            
        cv2.imshow('Pose Detection - RetinaFace', image_bgr)    
        
        # write original frames to disk
        if video_save:
            frame_v2 = cv2.resize(image_rgb, (640, 480))
            video_out.write(frame_v2)        
